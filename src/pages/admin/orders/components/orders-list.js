  ...
  async render() {
    ...
    return /* html */`
      ...
      <tr class="hover text-center">
        ...
        <td class="name-category font-semibold ${order.status === 'Pending' ? 'text-rose-600 ' : ''} ${order.status === 'Received' ? 'text-yellow-500 ' : ''} ${order.status === 'Completed' ? 'text-green-600 ' : ''} ">${order.status}</td>
        <td class="text-right">
          <div class="dropdown dropdown-end">
            ...
            <ul tabindex="0" class="p-2 shadow menu dropdown-content border-gray-200 bg-base-100 rounded-box w-52">
              <li>
                <a href="/admin/orders/details/${order.id}" class="active:bg-cyan-500">View details</a>
              </li> 
              <li>
